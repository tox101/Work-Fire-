// Self-hosted storage helper. Local filesystem storage is the default.
// Optional Forge storage remains available only when STORAGE_MODE=forge.
...
export async function storagePut(
  ...
  if (ENV.storageMode === "local") {
    ...
    return { key, url: `/files/${encodeURI(key)}` };
  }
...
export async function storageGet(relKey: string): Promise<{ key: string; url: string }> {
  ...
  return { key, url: ENV.storageMode === "local" ? `/files/${encodeURI(key)}` : `/manus-storage/${key}` };
}

export async function storageGetSignedUrl(relKey: string): Promise<string> {
  if (ENV.storageMode === "local") return `/files/${encodeURI(normalizeKey(relKey))}`;
  const { forgeUrl, forgeKey } = getForgeConfig();
  const key = normalizeKey(relKey);
