...
const scheduleType = z.enum(["task", "meeting", "personal", "review"]);
...
export const workspaceRouter = router({
  ...
  createSchedule: protectedProcedure.input(z.object({ title: z.string().trim().min(1).max(220), taskId: optionalTaskId.optional(), scheduleType: scheduleType.optional(), scheduleFlags: z.array(z.string().max(40)).max(12).optional(), plannedStartAt: z.date().nullable().optional(), plannedEndAt: z.date().nullable().optional(), notes: z.string().max(4000).nullable().optional() })).mutation(async ({ ctx, input }) => {
    const db = await databaseOrThrow();
    const taskId = input.taskId || null;
    await assertOptionalLinks(ctx.user.id, { taskId });
    const [created] = await db.insert(schedules).values({ userId: ctx.user.id, title: input.title, taskId, scheduleType: input.scheduleType ?? "task", scheduleFlags: input.scheduleFlags ?? [], plannedStartAt: input.plannedStartAt ?? null, plannedEndAt: input.plannedEndAt ?? null, notes: input.notes ?? null }).$returningId();
  ...
  updateSchedule: protectedProcedure.input(z.object({ id: z.number().int().positive(), expectedRevision: z.number().int().positive().optional(), title: z.string().trim().min(1).max(220).optional(), taskId: optionalTaskId.optional(), scheduleType: scheduleType.optional(), scheduleFlags: z.array(z.string().max(40)).max(12).optional(), plannedStartAt: z.date().nullable().optional(), plannedEndAt: z.date().nullable().optional(), notes: z.string().max(4000).nullable().optional() })).mutation(async ({ ctx, input }) => {
    const db = await databaseOrThrow();
