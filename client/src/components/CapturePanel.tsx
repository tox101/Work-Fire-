...
import { FileText, Paperclip, Tag, X } from "lucide-react";
...
export function CapturePanel({ workspace, onComplete, compact = false }: { workspace?: WorkspaceData; onComplete?: () => void; compact?: boolean }) {
  ...
  const [recordMode, setRecordMode] = useState<"issue" | "review">("issue");
  ...
  return (
    <form onSubmit={submit} className={`bg-white/90 ${compact ? "rounded-xl border border-violet-100 p-3" : "block-shadow border border-violet-100 p-4"}`}>
      ...
      <div className="grid grid-cols-2 gap-2" role="tablist" aria-label="오늘 기록 종류"><button type="button" onClick={() => setRecordMode("issue")} className={`rounded-xl border-2 px-3 py-3 text-left text-sm font-black ${recordMode === "issue" ? "border-violet-500 bg-violet-50 text-violet-900" : "border-slate-200 bg-white text-slate-500"}`}>오늘 이슈</button><button type="button" onClick={() => setRecordMode("review")} className={`rounded-xl border-2 px-3 py-3 text-left text-sm font-black ${recordMode === "review" ? "border-emerald-500 bg-emerald-50 text-emerald-900" : "border-slate-200 bg-white text-slate-500"}`}>오늘 복기</button></div><textarea autoFocus value={content} onChange={event => setContent(event.target.value)} className="mono-input mt-3 min-h-[336px] resize-y text-base leading-7" placeholder={recordMode === "issue" ? "오늘 발생한 이슈와 해결할 일을 적어주세요." : "오늘 잘한 점과 개선할 점을 적어주세요."} aria-label={recordMode === "issue" ? "오늘 이슈 내용" : "오늘 복기 내용"} />
