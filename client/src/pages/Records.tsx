...
function formatRecordDate(value: Date | string) { const date = new Date(value); const yy = String(date.getFullYear()).slice(-2); const month = String(date.getMonth() + 1).padStart(2, "0"); const day = String(date.getDate()).padStart(2, "0"); const weekday = new Intl.DateTimeFormat("ko-KR", { weekday: "short" }).format(date); const time = new Intl.DateTimeFormat("ko-KR", { hour: "2-digit", minute: "2-digit", hour12: false }).format(date); return `${yy}.${month}${day} ${weekday} ${time}`; }
...
export default function Records() {
  const [query, setQuery] = useState(""); const [projectFilter, setProjectFilter] = useState(ALL); const [taskFilter, setTaskFilter] = useState(ALL); const [sourceFilter, setSourceFilter] = useState<SourceType | typeof ALL>("capture"); const [period, setPeriod] = useState<"all" | "lastMonth" | "month">("month"); const [sort] = useState<"newest">("newest"); const [tagFilter, setTagFilter] = useState(ALL); const [selectedRecordId, setSelectedRecordId] = useState<number | null>(null); const [recordView, setRecordView] = useState<"idea" | "today" | "project" | "tag">("idea");
  const [day] = useState(() => new Date()); const monthRange = useMemo(() => ({ start: new Date(day.getFullYear(), day.getMonth(), 1), end: new Date(day.getFullYear(), day.getMonth() + 1, 1) }), [day]); const todayRange = useMemo(() => ({ start: new Date(day.getFullYear(), day.getMonth(), day.getDate()), end: new Date(day.getFullYear(), day.getMonth(), day.getDate() + 1) }), [day]);
  ...
  const search = trpc.workspace.recordSearch.useQuery({ query: query.trim() || undefined, projectId: projectFilter === ALL ? null : Number(projectFilter), taskId: taskFilter === ALL ? null : Number(taskFilter), sourceType: recordView === "today" ? "journal" : null, start: searchRange.start, end: searchRange.end, sort, tag: recordView === "tag" && tagFilter !== ALL ? tagFilter : null });
  ...
  const visibleRecords = (search.data ?? []).filter(record => recordView === "today" ? record.sourceType === "journal" : recordView === "idea" ? !isTodayRecord(record) && !record.projectId && !record.taskId && !record.projectTitle && !record.stageTitle && !record.taskTitle : recordView === "project" ? Boolean(record.projectId || record.taskId || record.projectTitle || record.stageTitle || record.taskTitle) : record.tags.length > 0);
