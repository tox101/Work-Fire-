...
export default function Records() {
  ...
  const search = trpc.workspace.recordSearch.useQuery({ query: query.trim() || undefined, projectId: projectFilter === ALL ? null : Number(projectFilter), taskId: taskFilter === ALL ? null : Number(taskFilter), sourceType: null, start: recordView === "today" ? null : searchRange.start, end: recordView === "today" ? null : searchRange.end, sort, tag: recordView === "tag" && tagFilter !== ALL ? tagFilter : null });
  ...
  const visibleRecords = (search.data ?? []).filter(record => recordView === "today" ? new Date(record.createdAt).toDateString() === day.toDateString() : recordView === "idea" ? !isTodayRecord(record) && !record.projectId && !record.taskId && !record.projectTitle && !record.stageTitle && !record.taskTitle : recordView === "project" ? Boolean(record.projectId || record.taskId || record.projectTitle || record.stageTitle || record.taskTitle) : record.tags.length > 0);
