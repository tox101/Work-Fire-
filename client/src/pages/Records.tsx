    ...
    <div className="space-y-3 max-w-6xl mx-auto pb-16">
      ...
      <section aria-label="Record 검색 및 필터" className="rounded-xl border border-slate-200 bg-white p-2.5 sm:p-3 shadow-2xs">
        ...
        <div className="mt-2 grid grid-cols-2 gap-1.5 sm:grid-cols-3 lg:grid-cols-6">
          <select aria-label="Record Project 필터" value={projectFilter} onChange={event => { setProjectFilter(event.target.value); setTaskFilter(ALL); }} className="h-9 rounded-lg border border-slate-200 bg-white px-2.5 text-xs font-bold text-slate-800"><option value={ALL}>전체 Project</option>{projects.map(project => <option key={project.id} value={project.id}>{project.title}</option>)}</select>
          <select aria-label="Record Task 필터" value={taskFilter} onChange={event => setTaskFilter(event.target.value)} className="h-9 rounded-lg border border-slate-200 bg-white px-2.5 text-xs font-bold text-slate-800"><option value={ALL}>전체 Task</option>{tasks.map(task => <option key={task.id} value={task.id}>{task.title}</option>)}</select>
          <select aria-label="Record 유형 필터" value={sourceFilter} onChange={event => setSourceFilter(event.target.value)} className="h-9 rounded-lg border border-slate-200 bg-white px-2.5 text-xs font-bold text-slate-800">{sourceOptions.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select>
          <select aria-label="Record 기간 필터" value={period} onChange={event => setPeriod(event.target.value as "all" | "month")} className="h-9 rounded-lg border border-slate-200 bg-white px-2.5 text-xs font-bold text-slate-800"><option value="all">전체 기간</option><option value="month">이번 달</option></select>
          <select aria-label="Record 태그 필터" value={tagFilter} onChange={event => setTagFilter(event.target.value)} className="h-9 rounded-lg border border-slate-200 bg-white px-2.5 text-xs font-bold text-slate-800"><option value={ALL}>전체 태그</option>{(tagOptions.data ?? []).map(tag => <option key={tag} value={tag}>#{tag}</option>)}</select>
          <select aria-label="Record 정렬" value={sort} onChange={event => setSort(event.target.value as "newest" | "oldest" | "pinned")} className="h-9 rounded-lg border border-slate-200 bg-white px-2.5 text-xs font-bold text-slate-800"><option value="newest">최신순</option><option value="oldest">오래된순</option><option value="pinned">고정 우선</option></select>
        </div>
        {hasFilter && (
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        ...
        <section aria-label="저장된 Record 검색" className="rounded-xl border border-slate-200 bg-white p-2.5 shadow-2xs">
          ...
          {savedSearches.data?.length ? <ul className="mt-2 space-y-1 border-t border-slate-100 pt-2">{savedSearches.data.map(savedSearch => <li key={savedSearch.id} className="flex min-w-0 items-center gap-1 rounded bg-slate-50 px-2 py-1"><button type="button" onClick={() => applySavedSearch(savedSearch)} className="min-w-0 flex-1 truncate text-left text-xs font-bold text-emerald-800 hover:underline">{savedSearch.name}</button><button type="button" onClick={() => deleteSearch.mutate({ id: savedSearch.id })} disabled={deleteSearch.isPending} aria-label={`${savedSearch.name} 저장 검색 삭제`} className="shrink-0 rounded p-1 text-slate-400 hover:bg-rose-50 hover:text-rose-700 disabled:opacity-40"><Trash2 className="h-3.5 w-3.5" /></button></li>)}</ul> : <p className="mt-2 text-[11px] text-slate-400">저장된 검색이 없습니다.</p>}
        </section>
      </div>
          ...
          <ul className="mt-2 space-y-2">
            {search.data.map(record => (
              <li key={record.id} className="rounded-lg border border-slate-100 bg-slate-50/70 p-2.5 hover:border-slate-300 transition-colors">
                ...
                <p className="mt-1.5 line-clamp-3 whitespace-pre-wrap text-sm font-semibold leading-5 text-slate-900">{record.content}</p>
