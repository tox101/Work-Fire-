...
export default function Records() {
  ...
  const visibleRecords = (search.data ?? []).filter(record => recordView === "today" ? true : recordView === "idea" ? !isTodayRecord(record) && !record.projectId && !record.taskId && !record.projectTitle && !record.stageTitle && !record.taskTitle : recordView === "project" ? Boolean(record.projectId || record.taskId || record.projectTitle || record.stageTitle || record.taskTitle) : record.tags.length > 0);
